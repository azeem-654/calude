   ...
   a browser closed and opened again. */
{
  ...
  const inApp = async (page) => (await page.getByLabel('Password', { exact: true }).count()) === 0 && (await page.getByText(/Dashboard|AI Autopilot/).count()) > 0;
  ...
  ok('an ended session returns to the sign-in screen', (await p2.getByLabel('Password', { exact: true }).count()) > 0);
